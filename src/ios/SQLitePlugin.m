...
@implementation SQLitePlugin : NSObject
...
-(void) exec: (NSArray *)command withBlock:(void (^)(NSMutableArray *))callback{
    ...
    logDebug(@"exec()");
    [self runInBackground:^{
        [self execOnBackgroundThread: command withBlock: callback];
    }];
}

-(void) execOnBackgroundThread: (NSArray *)command {
    [self execOnBackgroundThread:command withBlock: nil];
}

-(void) execOnBackgroundThread: (NSArray *)command withBlock:(void (^)(NSMutableArray *))callback{
    logDebug(@"execOnBackgroundThread()");
    NSString *dbName = [command objectAtIndex:0];
    NSArray *sqlQueries = [command objectAtIndex:1];
    BOOL readOnly = [[command objectAtIndex:2] boolValue];
...
// nicked from https://github.com/apache/cordova-ios/blob/92a62997adb3c8512328d5a0ae42fe5d156291f1/CordovaLib/Classes/Public/CDVCommandDelegateImpl.m
